...
def reweight_CE(data,hist_min,binsX,discX,binsY,discY,dV,T,fit):
   ...
   # print np.max(hist2)
    ...
    nbinsX = len(hist2[:,0])
    nbinsY = len(hist2[0,:])
    # print hist_max, type(int(hist_max))
    # print '====='*20
    ...
    dV_mat = np.zeros((nbinsX,nbinsY,int(hist_max))) # matrix for storing dV of each assigned
    ...
    for i in range(len(data[:, 0])):
        ...
        binfX[i] = jx
        binfY[i] = jy
        # print nA[jx, jy], type(nA[jx, jy]), nA[jx, jy].shape
        # print dV_mat, type(dV_mat), dV_mat.shape
        # print '====='*20
